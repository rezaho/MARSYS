...
TOPOLOGY
  ...
  "R". AgentD holds "S". Orchestrator dispatches all four workers in
  parallel, then assembles "MARS" from their replies.
...
def _build_agents(model_config: ModelConfig) -> Dict[str, Agent]:
    orchestrator = Agent(
        ...
        name="Orchestrator",
        goal="Dispatch AgentA/B/C/D in parallel, then assemble the secret word from their replies.",
        instruction=(
            "You run in TWO turns.\n"
            "TURN 1 (first call, no prior results): make a single `invoke_agent` "
            "tool call containing FOUR invocations — one for AgentA, one for AgentB, "
            "one for AgentC, one for AgentD. Each invocation's `request` field must be "
            ...
            "letter from the aggregated results (letters from AgentA, AgentB, AgentC, AgentD), "
    ...
    agent_c = Agent(
        model_config=model_config,
        name="AgentC",
        goal="Return letter R to the Orchestrator.",
        instruction=(
            "Your secret letter is R. Your ONLY action: make a single `invoke_agent` "
            "call with one invocation: agent_name='Orchestrator', "
            "request='Letter from AgentC: R'. Do NOT write any reasoning. "
            "Do NOT create plans. Call `invoke_agent` immediately."
        ),
        memory_retention="single_run",
    )
    agent_d = Agent(
        model_config=model_config,
        name="AgentD",
        goal="Return letter S to the Orchestrator.",
        instruction=(
            "Your secret letter is S. Your ONLY action: make a single `invoke_agent` "
            "call with one invocation: agent_name='Orchestrator', "
            "request='Letter from AgentD: S'. Do NOT write any reasoning. "
            "Do NOT create plans. Call `invoke_agent` immediately."
        ),
        memory_retention="single_run",
    )
