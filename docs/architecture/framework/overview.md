...
!!! info "Updated for v0.3.0"
    The execution stack documented here was rebuilt in commits `bc19b98` (Phase 3 cutover) and `2689a39` … `402b223` (Phase 3.5). `BranchSpawner` / `DynamicBranchSpawner` / `BranchExecutor` were replaced by `Orchestrator` (event loop) + `RealRuntime` (per-branch driver). Three reserved det-node singletons (`StartNode`, `EndNode`, `UserNode`) now drive workflow entry, exit, and human-in-the-loop. See [ADR-006](decisions/ADR-006-deprecation-timeline.md) for the full v0.2.x → v0.3.0 migration table.
...
| `DeterministicNode` family | `src/marsys/coordination/execution/det_nodes.py` | Reserved singletons `StartNode` / `EndNode` / `UserNode`. Non-LLM nodes with explicit lifecycle hooks. | TRUNK-CRITICAL |
...
2. **`Orchestra.execute`** invokes `_apply_legacy_topology_shim` (`orchestra.py:296`) to translate any `entry_point` / `exit_points` / legacy `User(Node)` metadata into explicit `Start` / `End` / `User` det-node edges (with `DeprecationWarning` per legacy concept). It then constructs `ValidationProcessor`, `Router`, `RulesEngine`, `RealRuntime`, and `Orchestrator`.
...
- **`DeterministicNode`** — non-LLM node with three lifecycle hooks: `on_workflow_start`, `on_single_invoke`, `on_dispatch`. Reserved singletons: `StartNode` (`det_nodes.py:67`), `EndNode` (`:96`), `UserNode` (`:119`). Reserved names live in `RESERVED_DETNODE_NAMES` (`:174`).
...
- **Add det-node types** — subclass `DeterministicNode` (`coordination/execution/det_nodes.py`), add the name to `RESERVED_DETNODE_NAMES`, implement the lifecycle hooks. Add gating helpers on `TopologyGraph` if the new det-node should expose a coordination tool to upstream agents.
