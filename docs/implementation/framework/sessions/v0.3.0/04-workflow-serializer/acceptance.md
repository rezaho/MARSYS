...
- AC-59: `NonSerializableTopologyError` is raised by `workflow_to_pydantic` when a `topology.nodes` entry is a `DeterministicNode` (instance of `StartNode`, `EndNode`, or `UserNode` from `coordination/execution/det_nodes.py`). Det-nodes carry execution-runtime state beyond what `NodeSpec` captures, so the wire shape refuses them explicitly rather than producing a partial spec. Tests demonstrate this by constructing a topology with at least one `DeterministicNode` and asserting the exception is raised with a message that names the offending node and points the caller at the workaround (drop the det-node before serializing, or open a separate PR to extend the spec).
  - _[REVERSED 2026-05-17 — Session 08; ADR-008 Decision 8]_ **This criterion is intentionally and explicitly reversed.** The premise ("det-nodes carry execution-runtime state beyond what `NodeSpec` captures") was refuted from primary source: `StartNode`/`EndNode` are `name`-only and `UserNode`'s only non-trivial field is a registry-injected handler (the same pattern as agent tools). Per ADR-008, `NodeType`→`NodeKind`, the wire is now **total** over all kinds (Start/End/User serialize by their `kind` discriminator), `NonSerializableTopologyError` is **deleted**, and the Session-04 tests that asserted `pytest.raises(NonSerializableTopologyError)` are **inverted to assert a successful round-trip** (no skip/xfail — surfaced here per anti-pattern #1, not a silent test deletion). New contract: `docs/implementation/framework/sessions/v0.3.0/08-unified-node-kind-model/acceptance.md` AC-20/AC-56. Reason: the Session-04 serializer mirrored the legacy enum-only model and banned the canonical det-nodes the v0.3 redesign had already made authoritative (`DEPRECATIONS.md:24-31`)._
...
- ~~AC-59 trigger conditions~~ **RESOLVED 2026-05-12**, then **REVERSED 2026-05-17 (Session 08, ADR-008 Decision 8)** — `NonSerializableTopologyError` is deleted; the wire is total over `NodeKind`. See the AC-59 amendment above.
