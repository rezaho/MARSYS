...
| 03 | **Visual builder** ✅ shipped | Vite + React + TanStack Router with the "Sunrise" design system (Geist Sans/Mono, warm-white ground + magenta + coral, light-mode primary, the Spren orb as the home-page interface). v0.3 home is orb-only — the four-surface command center (Now / Since-you-were-away / Activity / Chat) is deferred to Session 09 to avoid shipping placeholder surfaces that contradict SP-007. `@xyflow/react` canvas with custom node types per `NodeType`; right-rail agent config form; pattern preset insertion (HUB_AND_SPOKE, PIPELINE, HIERARCHICAL, MESH); topology compile + Spren-side semantic lint adapter with inline issue display | B1+B2+B3 |
| 04 | **Run execution + AG-UI streaming + cost** | `POST /v1/runs` background-task execution calling `Orchestra.run()` with a frozen-at-run-start workflow snapshot; `GET /v1/runs/{id}/events` SSE wrapping the framework's `AGUIEventStream(orchestra, run_id)` async iterator (translator ships framework-side at `marsys.transport.aggui` in Framework v0.3 Session 06, NOT Spren-side); aggregate `GET /v1/runs/events` SSE for the run-list page; cost rate table YAML + per-run aggregation; canvas Run button wiring with token counter + cancel UX (5s user-visible countdown, 4s internal force-abort); thin `/runs/{id}` placeholder that Session 05 replaces | C1+C2 |
| 05 | **Run inspection** | `GET /v1/runs/{id}/trace` reading `<data-dir>/data/runs/{id}/trace.ndjson` and returning a hierarchical span tree; `GET /v1/runs/{id}/workflow` returning the frozen snapshot; `POST /v1/files` multipart upload + `<data-dir>/data/files/{file_id}/<original_name>` storage + size/storage caps + sha256 + delete-reference-guard; `POST /v1/runs` non-empty attachments resolution; `/runs/{id}` full inspector (TraceTree + right-drawer SpanDetailPanel + Attachments + Workflow-as-run accordion); canvas-side `📎` attach affordance with drag-and-drop; run history filter rail (date range + multi-status + workflow filter) on `/runs` | C3+D1+D2 |
...
| 09 | **Home page UI — full four-surface command center** | Expands the orb-only home (shipped in Session 03 as a v0.3 transitional landing) into the full four-surface command center: Now / Since-you-were-away / Activity / Chat input. Suggestion confirm flow; activity stream; chat input → meta-agent SSE roundtrip (wired live to the meta-agent shipped in Sessions 07 + 08); cost meter + budget status in header. The orb remains the brand anchor but now reflects the meta-agent's reactive state (idle / thinking / speaking) against live work, not a stubbed delay | G1+G2 |
...
See [`v0.4-extensions.md`](./v0.4-extensions.md). Roughly 30 sessions covering: TUI client (Textual), Tauri polish (tray / autostart / OS notifications / deep links / file associations), sub-instance spawning + lifecycle, teams + team managers, full skills catalog, Tier 4 hybrid index + recall tool, daily consolidation pipeline, standing approvals, channel adapters (Telegram polling + Discord gateway), cron scheduler, webhook + cloudflared, workflow templates + import/export, `SprenTelemetrySink` consumer (in `packages/spren/src/spren/telemetry/` for the `python my_workflow.py → Spren UI` flow), advanced semantic linter consumption, pause/resume Spren surface (consuming Framework Session 03's primitives), **Phase AA — Custom Tooling**: user-authored custom tools with a file-backed registry + in-app code editor + sandboxed execution, plus meta-agent-generated custom tools with a hard-rail approval flow.
...
Spren-driven framework features are tracked in [`../framework/`](../framework/) — a parallel directory mirroring this Spren one. Per-version plans and per-session framework PR briefs. Framework features blocking Spren v0.3:

- **Framework Session 01** — NDJSON streaming tracing writer ([brief](../framework/sessions/v0.3.0/01-ndjson-streaming-tracing-writer.md)). Shipped on `feature/tracing-streaming`.
- **Framework Session 06** — AG-UI event-stream translator at `marsys.transport.aggui` ([brief](../framework/sessions/v0.3.0/06-aggui-translator.md)). Required before Spren Session 04 implementation begins.

Framework features Spren v0.4 builds on (already shipped on `feature/tracing-streaming`, awaiting v0.4 Spren-side consumers):

- **Framework Session 02** — `TelemetrySink` Protocol + `SecretRedactor`. Spren consumes via `SprenTelemetrySink` in v0.4-27.
- **Framework Session 03** — Pause/resume completion (`quiesce` / `snapshot` / `restore_from` / `resume` on `Orchestrator`; `Orchestra.pause_session` / `resume_session` / `list_paused_sessions` / `discard_paused_session`). Spren surfaces in v0.4-29.

Framework features still in flight:

- **Framework Session 04** — Workflow definition serializer. Spren Session 02 mirrors the marsys types in the meantime; a 24-hour cleanup PR drops the mirrors when Framework 04 merges.
- **Framework Session 05** — Advanced semantic linter. Held until Spren Session 03 shipped; can now rewrite the brief against the concrete consumer needs.
