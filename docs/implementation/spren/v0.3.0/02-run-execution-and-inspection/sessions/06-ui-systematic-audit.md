...
| 2026-05-18 — autonomous fix sweep (user asleep) | 5 fixes shipped on `feature/spren-umbrella`, each = code + tests + RCA/solution doc, conventional commits, NOT merged to main: **RUN-1** (`8ed4bb2`, typed `ApiError` + envelope parser; create failures now surface via the completion-toast channel; rerun consumer migrated to `.code`/`.status`), **RUN-3b** (`1e95206`, app-wide `INTERNAL_ERROR` envelope handler + `_summarize_failure` reads real per-branch reason from the public `OrchestraResult.branch_results`), **LINT-REACTIVITY** (`dc02101`, lint endpoint now lints the *submitted* body not stored state — independent reviewer caught a missed scenario-harness consumer, fixed; contract change flagged for sign-off), **SWEEPER-1** (`dcdbafa`, both sweep + list predicates exclude workflows-with-runs; test-first), **RIGHTRAIL-1** (`75234cd`, pinned sticky action footer + rail stacks above orb — functional half; not browser-verified, flagged). RUN-3e consolidated into a one-read **DECISION NEEDED** block (`109192a`) — held for user (opaque-vs-transparent prompt-enrichment is a product call; recommendation = transparent enrichment). Verified-pre-existing flake `test_put_replaces_workflow` (microsecond `updated_at` on Windows) confirmed unrelated. Deferred (need user opinion): RUN-3e impl, palette PALETTE-2/4 (spec-vs-shipped-flat conflict), NODE-1, RIGHTRAIL-2, nit sweep #18. |
