...
| Spren Session 04 (run execution + AG-UI streaming + cost) | shipped | Per-run NDJSON trace file at `<data-dir>/data/runs/{run_id}/{trace_id}.ndjson` (glob-resolved by Spren — single file per run via per-run `TracingConfig.output_dir`). Session 06's session log (Tier 2) coexists with the per-run trace (different scopes — Tier 2 is daemon-wide; the per-run trace is run-scoped). |
| Spren Session 05 (run inspection) | shipped | `GET /v1/runs/{id}/trace` reads the per-run NDJSON via `runs/trace.py`. Adds `<data-dir>/data/files/` for user uploads + a `files` table; Session 06's bootstrap path runs alongside but does not interact with these. The `app-shell AuthGate` shipped after Session 05 wraps every route — meta-agent UI surfaces in Session 07/08 will already be inside the gate. |
...
**`_self.md` seed:** the `settings` table is spec'd in `02-data-model.md` but is NOT yet shipped (Sessions 01-05 added `workflows`, `runs`, `_idempotency`, `files`; the settings table is a Session 06 concern). Bootstrap behavior: if the table exists AND has `user.name` / `user.pronouns` rows, fill them in; otherwise `_self.md` is written with empty frontmatter and a comment line: `# This is your identity record. Tell Spren your name and it'll be filled in.` First user-message that contains a `name` extraction will populate this via the live-commit path.

The implementer either (a) ships the `settings` table as part of Session 06's migrations (the simplest path; the `key TEXT PRIMARY KEY, value TEXT, updated_at TEXT` shape from `02-data-model.md` §`settings`), or (b) ships only the bootstrap-without-settings-read path and defers the table to Session 07. The brief recommends (a) since the table is needed for the v0.3 settings surface anyway and it's a single 5-line migration.
...
| Path | Purpose |
|---|---|
| `packages/spren/src/spren/storage/migrations/04__create_events_table.py` | Forward-only. `events` + `events_fts` + indexes. (Session 05 took `03__create_files.py`; Session 06 starts at `04__`.) |
| `packages/spren/src/spren/storage/migrations/05__create_memory_tables.py` | `entities`, `facts`, `pending_facts`, `prose_chunks` (FTS5). |
| `packages/spren/src/spren/storage/migrations/06__create_settings_table.py` | `settings (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TEXT NOT NULL)` per `02-data-model.md` §`settings`. Required for the `_self.md` seed path; folded into Session 06 since the bootstrap reads from it. (Optional — if the implementer takes path (b) above and defers, drop this row.) |
...
| `packages/spren/src/spren/server.py` | Compose with the existing lifespan (Sessions 01-05 already wired the migration runner, idempotency sweep, `warn_if_rates_stale`, the draft sweeper task, `shutdown_all_active(timeout=5.0)` on the shutdown side, and the `RunsBroker` singleton). Session 06 adds: `memory.bootstrap.ensure_kb_exists(sandbox)` on startup; starts the `SprenMarkdownIndexer` background task; starts the `PendingFactsWatcher` background task; writes the token + port files for the CLI bridge (decision §11) on startup, deletes them on shutdown. Both new background tasks join the existing cancel-on-shutdown pattern (cancel; await with timeout; ignore `CancelledError`). Don't replace the existing hook — extend it. |
| `packages/spren/src/spren/__main__.py` | (Or wherever cleanest) Write `<data-dir>/runtime/auth-token` and `<data-dir>/runtime/spren-port` atomically + mode 0600 at sidecar startup, BEFORE uvicorn binds the listener. Delete both on shutdown. Same atomic-write primitives the SessionLog uses (write-temp + fsync + rename + parent-dir-fsync). The Tauri shell + browser-fragment paths are unchanged; the file is purely for the CLI bridge to read. |
...
11. **CLI bridge: TCP-localhost-with-token across all platforms.** The `spren memory` CLI talks to the running daemon over `http://127.0.0.1:<port>` using the per-launch auth token — same pattern as the FastAPI clients (Tauri webview, browser tab, future TUI). Rejected: per-platform IPC (Unix sockets on macOS/Linux + named pipes on Windows). Reasoning: the CLI runs a few commands per minute, not thousands per second; localhost-TCP overhead is microseconds (irrelevant); IPC's marginal correctness isn't worth maintaining two platform-specific code paths. The CLI just becomes another HTTP client.

   **Auth-token persistence** (NOT yet shipped — Session 06 ships it as part of this decision): Session 01 prints the token to stdout but does NOT persist it to disk; the Tauri shell + browser fragment-injection consume the stdout line directly. The CLI runs in a different process and can't read the daemon's stdout, so Session 06 must add a token-file write at sidecar startup. Path: `<data-dir>/runtime/auth-token` (mode 0600), `<data-dir>/runtime/spren-port` (mode 0600). Files are written atomically (write-temp + rename + parent-dir-fsync) on `lifespan` startup AFTER the FastAPI app is wired but BEFORE uvicorn binds — so the file lands before any spawned CLI tries to read it. Files are deleted on `lifespan` shutdown so a stale token from a crashed daemon doesn't auth a new daemon. CLI reads both, builds the URL, passes `Authorization: Bearer <token>`. Refuses with a clear error ("Spren daemon is not running. Start it with `just dev` and try again.") if either file is missing or unreadable. **In-scope addition for Session 06**: extend `__main__.py` / `server.py` to write + delete the token + port files; document in `07-security.md` that this file is per-launch and cleared on shutdown.
