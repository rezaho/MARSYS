...
class Message:
    ...
    tool_call_id: Optional[str] = (
        None  # For tool role providing result for a specific tool_call_id
    )
    ...
    @classmethod
    def from_model_response_dict(
        cls, data: Dict[str, Any], default_id: Optional[str] = None
    ) -> "Message":
        """Creates a Message object from a model's response dictionary (e.g., OpenAI's message object)."""
        # data is a dict like {"role": "assistant", "content": "...", "tool_calls": ...}
        return cls(
            role=data["role"],
            content=data.get("content"),  # May be None
            name=data.get("name"),
            tool_calls=data.get("tool_calls"),
            tool_call_id=data.get(
                "tool_call_id"
            ),  # Usually not in model response, but for completeness
            message_id=default_id
            or str(uuid.uuid4()),  # Generate new ID for this new message from model
        )
...
class ConversationMemory(BaseMemory):
    ...
    def replace_memory(
        ...
        *,
        role: Optional[str] = None,
        content: Optional[str] = None,
        message_id: Optional[str] = None,  # ID of the message being replaced, or new ID
        name: Optional[str] = None,
        tool_calls: Optional[List[Dict[str, Any]]] = None,
        tool_call_id: Optional[str] = None,
    ) -> None:
