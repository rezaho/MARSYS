...
class ReflexiveStateTrackingRule(Rule):
    """
    Tracks when agents are called via reflexive edges.
    
    This rule detects transitions on reflexive edges and marks the target agent
    as being in a reflexive call, storing the caller in branch metadata.
    """
    ...
    def __init__(
        ...
        priority: RulePriority = RulePriority.CRITICAL  # Run before other rules
    ):
        """
        Initialize reflexive state tracking rule.
        
        Args:
            source_agent: The agent initiating the reflexive call
            target_agent: The agent being called reflexively
            name: Optional rule name
            priority: Rule priority (highest by default to run first)
        """
        rule_name = name or f"reflexive_state_{source_agent}_to_{target_agent}"
        super().__init__(rule_name, RuleType.FLOW_CONTROL, priority)
        self.source_agent = source_agent
        self.target_agent = target_agent
    
    async def check(self, context: RuleContext) -> RuleResult:
        """Check if this is a reflexive edge transition and update state."""
        current = context.metadata.get("current_agent")
        target = context.metadata.get("target_agent")
        action = context.metadata.get("action_type")
        
        # Detect reflexive edge transition
        if (action == "invoke_agent" and 
            current == self.source_agent and 
            target == self.target_agent):
            
            # Mark in branch metadata that target agent is in reflexive call
            return RuleResult(
                rule_name=self.name,
                passed=True,
                action="modify",
                reason=f"Marking {self.target_agent} as reflexively called by {self.source_agent}",
                modifications={
                    "update_state": {
                        f"reflexive_caller_{self.target_agent}": self.source_agent
                    }
                }
            )
    ...
    def description(self) -> str:
        return f"Reflexive state tracking: {self.source_agent} -> {self.target_agent}"


class ReflexiveReturnRule(Rule):
    """
    Routes back to caller when reflexive agent completes (Boomerang Pattern).
    
    This rule ensures that agents invoked via reflexive edges (<=>)
    automatically return control to their caller upon completion.
    """
    ...
    def __init__(
        ...
        """
        Initialize reflexive return rule.
        
        Args:
            agent_name: The agent this rule applies to
            name: Optional rule name
            priority: Rule priority
        """
        rule_name = name or f"reflexive_return_{agent_name}"
        super().__init__(rule_name, RuleType.FLOW_CONTROL, priority)
    ...
    def description(self) -> str:
        return f"Reflexive return rule for {self.agent_name} (Boomerang Pattern)"
